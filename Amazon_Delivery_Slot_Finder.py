'''
The Amazon Delivery Slot Finder notifies users when it finds a delivery slot opened up on the checkout page. 
This is required since due to the Corona virus, delivery slots are few and disappear within seconds due to extremely high demand.
Our aim is to try and give the user of this programme a bit of a headstart when it comes to looking for and finalizing delivery slots.
#StrongerTogether

Copyright (C) 2020  Ojasvi Maleyvar
...
__credits__ = ["Shivangi Gupta", "Vivek Gautham"]
__license__ = "GPL"
__version__ = "1.0.0"
__maintainer__ = "Ojasvi Maleyvar"
__email__ = "ojasvi.maleyvar@gmail.com"
...
def long_operation_thread(driver, pushme_secret_tok, window):
    """
        ...
        if "shipoptionselect" in driver.current_url:
            ...
            while delivery_slot_found == False:
                ...
                if " AM " in page_text or " PM " in page_text or " AM" in page_text or " PM" in page_text:
                    ...
                    pushme_secret_tok = values['-Secret_Token-']
...
def the_gui():
    ...
    layout = [  
                ...
                #[sg.Text('Please enter your notification token: '), sg.InputText(key='-Secret_Token-')],
                [sg.Text('\n\n\n')],
                [sg.Text('\n\n\n')],
    ...
    while True:
        ...
        if event in (None, 'Exit'): # if user closes window or clicks cancel
            ...
            thread = threading.Thread(target=long_operation_thread, args=(driver, pushme_secret_tok, window), daemon=True)
            thread.start()
